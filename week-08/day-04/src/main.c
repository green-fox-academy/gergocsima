...
int main(void) {
	...
	int upper = 4500;
	int lower = 2050;
	char c[] = "Let's play a game! Are you ready?\r\n";
	int buttonStatus = 0;
	int mode = 0;
	int gameNr = 0;
	float totalMs = 0;
	...
	HAL_UART_Transmit(&uart_handle, &c, strlen(c), 500);

	while (1) {
		int randomNr = HAL_RNG_GetRandomNumber(&rng)
				% ((upper - lower + 1) + lower);
		...
		if (mode == 0) {
			...
			printf("Your reaction time is: %.2f seconds.\n", (result/1000));
			totalMs += result;
		}if (gameNr == 10) {
			printf("The average reaction time is %.2f seconds\n", (totalMs / 1000)/gameNr);
			gameNr = 0;
			totalMs=0;
		}
	}
